...
function fetchExpenses(page = 1) {
    ...
    axios.get(`http://13.49.76.67:3000/expense?page=${page}&limit=${itemsPerPage}`, { headers: { 'authorization': token } })
        .then((res) => {
...
expenseForm.addEventListener('submit', function(event) {
    ...
    axios.post('http://13.49.76.67:3000/expense', { amount, description, category }, { headers: { 'authorization': token } })
        .then((res) => {
...
function deleteExpense(expenseId) {
    const token = localStorage.getItem('token');
    axios.delete(`http://13.49.76.67:3000/expense/${expenseId}`, { headers: { 'authorization': token } })
        .then((res) => {
            fetchExpenses(currentPage);
        })
        .catch(err => {
...
//   console.log(token); 
  axios
    .get("http://13.49.76.67:3000/purchase/premiumMembership", {
    ...
    .then((response) => {
      var options = {
        ...
        handler: function (response) {
          axios
            .post(
              "http://13.49.76.67:3000/purchase/updateTransactionStatus",
                {
...
function checkPremiumStatus() {
    const token = localStorage.getItem('token');
    axios.get('http://13.49.76.67:3000/user/premiumStatus', { headers: { 'authorization': token } })
        .then((res) => {
...
function download() {
    const token = localStorage.getItem('token');
    axios.get('http://13.49.76.67:3000/user/download', { headers: { 'authorization': token } })
        .then((res) => {
            if (res.status === 200) {
...
function showLeaderboard(){  
    const token = localStorage.getItem('token');
    axios.get('http://13.49.76.67:3000/premium/leaderboard', { headers: { 'authorization': token } })
        .then((res) => {
...
function fetchDownloadHistory() {
    const token = localStorage.getItem('token');
    axios.get('http://13.49.76.67:3000/download/history', { headers: { 'authorization': token } })
        .then((res) => {
